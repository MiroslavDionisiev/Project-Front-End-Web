...
function renderBook(doc)
{
    ...
    var genreRef = storageRef.child("Thriller");
...
function addAuthor(name, value, isChecked)
{
    let checkbox = document.createElement('input');
        ...
        checkbox.addEventListener('change', function()
        {
            ...
            {
                db.collection('Books').doc("Thriller").collection('Titles').where('author','==',name).get().then(snapshot=>
                {
                    clearAuthors();
            ...
            {
                db.collection('Books').doc("Thriller").collection('Titles').get().then(snapshot=>
                {
                    clearAuthors();
                    listAuthors();
                    printBooks(snapshot);
